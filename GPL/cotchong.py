...
def read_json(filename, key_name):
    """Đọc file JSON và tổng hợp dữ liệu theo tháng."""
    from collections import defaultdict
    monthly_data = defaultdict(float)

    with open(filename, "r", encoding="utf-8") as f:
        try:
            data = json.load(f)  # Đọc toàn bộ file .json (là 1 danh sách)
            for entry in data:
                ...
                if value is None:
                    continue
                first_date = date_range.split(" to ")[0] if " to " in date_range else date_range
...
def read_data(filename, key_name):
    if filename.endswith(".jsonl"):
        return read_jsonl(filename, key_name)
    else:
        return read_json(filename, key_name)


optimized_data = read_data("results.json", "objective_value")
fcfs_data = read_data("FCFS.json", "fcfs_cost")
...
plt.title("Electricity Costs: Our Model vs FCFS by Month in JPL")
