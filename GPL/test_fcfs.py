...
def main():
    # 1) Khoảng ngày
    start_date = datetime.strptime("05-09-2018", "%d-%m-%Y")
    ...
    # 4) File ghi kết quả cho Optimized và FCFS (giữ nguyên cách ghi hiện tại)
    json_file_opt = "results.json"
    json_file_fcfs = "FCFS.json"
    
    results_opt = []
    results_fcfs = []
    
    # Danh sách lưu các block infeasible
    infeasible_results = []
    ...
    for idx, block in enumerate(blocks, 1):
        ...
        # Nếu trạng thái LP là infeasible, lưu thông tin block vào danh sách infeasible_results
        if status_opt == "Infeasible":
    ...
    with open(json_file_opt, "w", encoding="utf-8") as f_opt:
        json.dump(results_opt, f_opt, ensure_ascii=False, indent=2)
    with open(json_file_fcfs, "w", encoding="utf-8") as f_fcfs:
        json.dump(results_fcfs, f_fcfs, ensure_ascii=False, indent=2)
    with open("infeasible.json", "w", encoding="utf-8") as f_infeasible:
        json.dump(infeasible_results, f_infeasible, ensure_ascii=False, indent=2)

    # 7) In tổng hợp theo tháng
    ...
    # 8) Vẽ biểu đồ so sánh chi phí tối ưu theo tháng (Optimized)
    ...
    # 9) Vẽ biểu đồ so sánh chi phí FCFS theo tháng
