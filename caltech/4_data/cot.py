...
bar_width = 0.3  # Độ rộng của mỗi cột trong nhóm
...
rects2 = ax.bar(x - 0.5 * bar_width, consumption_without_values, bar_width, label="Optimal", color="green")
# rects3 = ax.bar(x + 0.5 * bar_width, consumption_A_values, bar_width, label="With Solar (FCFS)", color="purple")
rects4 = ax.bar(x + 0.5 * bar_width, consumption_B_values, bar_width, label="FCFS", color="orange")

# --- Cấu hình biểu đồ ---
ax.set_xlabel("Months")
ax.set_ylabel("Electricity Cost (EUR)")
ax.set_title("Monthly Electricity Cost Comparison (Optimal vs FCFS, Without Solar) in Caltech") # Cập nhật tiêu đề
