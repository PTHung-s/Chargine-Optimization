...
# --------------------------
consumption_with_renewable = load_json("monthly_no_solar_cost.json")
consumption_without_renewable = load_json("monthly_no_solar_splus.json")
cost_with_renewable = load_json("monthly_with_solar_cost.json")
cost_without_renewable = load_json("monthly_with_solar_splus.json")
...
plt.title("Electricity consumption by months with FCFS in Caltech")
...
plt.title("The cost of electricity over the months with FCFS in Caltech")
plt.xticks(rotation=45)
plt.legend()
plt.grid(True)
plt.gca().yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.0f}"))
