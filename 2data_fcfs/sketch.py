...
plt.title("Electricity consumption by months without ACN")
...
# --------------------------
plt.figure(figsize=(10, 5))
plt.plot(months, cost_without_values, label="Chi phí không dùng NL tái tạo", color="orange", linestyle="--", marker="o")
plt.plot(months, cost_with_values, label="Chi phí có dùng NL tái tạo", color="blue", linestyle="-", marker="s")
plt.xlabel("Months")
plt.ylabel("Total Cost (EURO)")
plt.title("The cost of electricity over the months without ACN")
plt.xticks(rotation=45)
plt.legend()
plt.grid(True)
plt.gca().yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.0f}"))
